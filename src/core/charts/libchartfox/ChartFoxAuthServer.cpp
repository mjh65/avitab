...
ChartFoxAuthServer::ChartFoxAuthServer() {
}

void ChartFoxAuthServer::setAuthCallback(AuthCallback cb) {
    onAuth = cb;
}

int ChartFoxAuthServer::start() {
    if (serverThread) {
        keepAlive = false;
        serverThread->join();
        serverThread.reset();
    ...
    keepAlive = true;
    serverThread = std::make_unique<std::thread>(&ChartFoxAuthServer::loop, this);

    return ntohs(chosenAddr.sin_port);
}

void ChartFoxAuthServer::loop() {
...
void ChartFoxAuthServer::handleClient(int client) {
    method.clear();
    url.clear();
    version.clear();
...
bool ChartFoxAuthServer::handleData(const std::string& data) {
    for (char c: data) {
        switch (state) {
        case State::METHOD:
            if (!isspace(c)) {
