...
std::vector<std::shared_ptr<navdb::Airport>> SqlLoadManager::getMatchingAirports(const std::string &pattern)
{
    ...
    std::vector<std::shared_ptr<navdb::Airport>> airports;
    for (auto id: ids) {
...
std::shared_ptr<navdb::Airport> SqlLoadManager::getAirport(const std::string &icao)
{
...
navdb::NodeList SqlLoadManager::getFixList(const std::vector<int> &fixKeys)
{
